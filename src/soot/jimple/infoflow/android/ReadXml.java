...
public class ReadXml {
		...
		try {
			...
			Enumeration entries = archive.entries(); entries.hasMoreElements();) {
				...
				if (entryName.matches(".*\\.xml")
					...
					// entryName.substring(entryName.lastIndexOf("/")+1,entryName.lastIndexOf(".xml")));
					String fileIdentifier = entryName.substring(
							entryName.lastIndexOf("/") + 1,
							entryName.lastIndexOf(".xml"));
					xmlFile = archive.getInputStream(entry);
					List<String> additionalEntryPointsMethodName = handleXmlFile(xmlFile);
					xmlFile.close();

					additionalEntryPoints.put(fileIdentifier,
							additionalEntryPointsMethodName);
	 ...
	 * 
	 * @param classes
	 *            all classes that exist in the Manifest file
	 * @return
	 */
	public List<String> getAdditionalEntryPoints(Set<String> classes) {
		...
		for (String set : classes) {
			for (Map.Entry<String, List<String>> elem : additionalEntryPoints
					.entrySet()) {
				for (String method : elem.getValue()) {
					returnList.add("<" + set + ": void " + method
							+ "(android.view.View)>");
				}
			}
		}
	...
	private List<String> handleXmlFile(InputStream xmlFile)
...
//				if (tagName.equals("Button") || tagName.equals("RadioButton")
...
//						|| tagName.equals("ImageButton")){
					...
					for (int i = 0; i < parser.getAttributeCount(); i++) {
						...
						if (attribute.equals("onClick")) {
							additionalEntryPointsMethodName.add(AXMLPrinter
