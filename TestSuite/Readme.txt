...
This app contains an access to a field in an array that is not tainted. This field is send via sms (no data leakage). The import ant
...
ServiceLifecycle:
The app contains a Service with three callback methods. One of them contains a source, another one contains a sink which receives value originated from the source as input parameter.
...
A static field gets tainted from a different activity. Both lifecycles have to be analyzed and combined to find this information flow.
